...
USER_MAX_QUOTA = 5 * 1024 * 1024 * 1024
...
def get_file_icon(category):
    """Get appropriate Bootstrap icon class for file category"""
    icon_map = {
    ...
    return icon_map.get(category, 'bi bi-file-earmark')
...
def get_storage_stats(user):
    ...
    total_size = 0
    storage_by_type = {'image': 0, 'video': 0, 'audio': 0, 'document': 0, 'other': 0}
...
def index():
    ...
    for item in collection.find(query).sort("is_folder", -1).sort("name", 1):
        ...
        if not item.get("is_folder", False):
            ...
            try:
                ...
                item["icon"] = get_file_icon(item["category"])
            except OSError:
                item["size"] = 0
    ...
    for category, size in storage_by_type.items():
        ...
        storage_breakdown[display_name] = {
            'size': size,
